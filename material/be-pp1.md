...
1. **Decide Initial Roles**:
  - Determine who will start as the driver and who will be the navigator. Remember to switch roles after each major step (for example, after each iteration).

2. **Get the Starter Code**:
  You should **start from the provided starter API**, because it already contains tests and configuration.

  - Clone the tours API repository:
    ```bash
    git clone https://github.com/tx00-resources-en/week4-be-pp-sample-sol week6-be-pp1
    cd week6-be-pp1
    rm -rf .git
    ```
  - **Explanation**: Removing the `.git` directory ensures that you start with a fresh Git history for your new repository.

  After this step, you should see files like `app.js`, `config/db.js`, `models/`, `controllers/`, `routes/`, and `tests/` inside the `week6-be-pp1` folder.
...
3. **Install Dependencies and Run the Server**:
   - Install the required dependencies and start the development server:
     ...
     npm run dev
     ```
  - If everything works, your terminal should show something like `Server is running on port 4000`.
  - You can do a quick check in a browser or Postman using:
...
1. **Database connection:**
  - In `app.js`, import and call `connectDB()` so your app connects to MongoDB:
  ...
  - Make sure `connectDB()` is called **before** `app.listen(...)`.
...
3. **Update the Tour Controllers**:
  ...
  - Replace any in-memory logic (arrays / old `tourLib` calls) with **Mongoose calls** to the `Tour` model you just defined.
  - For example:
    - `getAllTours` should use something like `Tour.find().sort({ createdAt: -1 })`.
    - `getTourById` should use `Tour.findById(tourId)`.
    - `createTour` should use `Tour.create(req.body)`.
    - `updateTour` should use `Tour.findByIdAndUpdate(tourId, req.body, { new: true })`.
    - `deleteTour` should use `Tour.findByIdAndDelete(tourId)`.
   - The general pattern is:
     - **Read data:** `Tour.find(...)` / `Tour.findById(...)`
     - **Create data:** `Tour.create(...)`
     - **Update data:** `Tour.findByIdAndUpdate(...)`
     - **Delete data:** `Tour.findByIdAndDelete(...)`
   - Wrap these calls in `try { ... } catch (error) { ... }` blocks and send a `500` status code on unexpected errors (see the cars starter for examples).
  - Refer to the [cars API example](https://github.com/tx00-resources-en/be-starter-v2) for full controller examples.
...
This iteration repeats the same pattern you used for **tours**, but now for **users**. You will:
- define a Mongoose schema for `User`,
- update the controllers to use the `User` model,
- and ensure the endpoints behave as described below.
...
2. **Update the User Controllers**:
  - Open `controllers/userControllers.js`.
  - Replace any in-memory logic with **Mongoose calls** to the `User` model:
    - `getAllUsers` → `User.find().sort({ createdAt: -1 })`.
    - `getUserById` → `User.findById(userId)`.
    - `createUser` → `User.create(req.body)`.
    - `updateUser` → `User.findByIdAndUpdate(userId, req.body, { new: true })`.
    - `deleteUser` → `User.findByIdAndDelete(userId)`.
  - Again, you can use the [cars API example](https://github.com/tx00-resources-en/be-starter-v2) as a reference.
...
> ⚠️ In a real API, we **would not return the password field** for security reasons. We keep it in these examples for clarity with the starter tests and will discuss best practices for handling sensitive data in a later backend session.
...
1. **Prepare the `.env` File**
  ...
  - In your project root (inside the `week6-be-pp1` folder), copy the example environment file `.env.example` to `.env`:
    ```bash
    cp .env.example .env
    ```
  - Open the new `.env` file and review its contents. You will find settings for the port and database URI:
     ```env
     PORT=4000
...
3. **Install and Configure `dotenv`**
  ...
  - Open `app.js` and add the following line at the top of the file to configure `dotenv` (before you use `process.env.PORT` or connect to the database):
     ```js
...
The file `middleware/customMiddleware.js` should export two middlewares: `unknownEndpoint` and `errorHandler`.
